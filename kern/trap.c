...
trap_init_percpu(void)
{
   ...
   // --- Old Lab 3 implementation when there was only 1 CPU ---
/*
...
trap_dispatch(struct Trapframe *tf)
{
   ...
   if (tf->tf_trapno == IRQ_OFFSET + IRQ_SERIAL) {
	...
	print_trapframe(tf);
	if (tf->tf_cs == GD_KT)
		panic("unhandled trap in kernel: trap #%d", tf->tf_trapno);
	else {
		env_destroy(curenv);
		return;
	}
...
trap(struct Trapframe *tf)
{
	...
	if ((tf->tf_cs & 3) == 3 || curenv->env_type == ENV_TYPE_FLEX) {
      ...
      // Handle special FlexSC kernel thread case
